...
export const COMMAND_DETAIL: Record<string, readonly string[]> = {
  install: [
    "At a terminal it asks your name, makes the store, connects Claude Code",
    "(a backup of ~/.claude/settings.json is kept; `counterparts disconnect`",
    "undoes it; --no-connect skips it), and offers two optional keys one at a",
    "time. Run it again any time: it keeps what is there, says `Welcome back`,",
    "and offers the keys again. If ~/.counterparts is gone and a",
    "~/.counterparts.parked-<date> is beside it, it asks first whether to bring",
    "that memory back or start blank.",
    "",
    "The Anthropic key is only used to write up a session that ended before the",
    "assistant could, and that conversation is sent to Anthropic; everything",
    "else works without it. The Voyage key lets recall match by meaning, not",
    "just by words, and sends memory text to Voyage; recall works without it.",
    "",
  ...
  credentials: [
    ...
    "",
    "ANTHROPIC_API_KEY is only used to write up a session that ended before the",
    "assistant could (that conversation is sent to Anthropic). VOYAGE_API_KEY",
    "lets recall match by meaning, not just by words (memory text is sent to",
    "Voyage), and setting it offers to turn that on. Both are optional.",
  ],
  rebrief: [
