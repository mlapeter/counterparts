...
22. **[M] Every other key and every other tool's hooks survive, and the merge works on
    ...
    *Amended 2026-09-23 (review n1): and the file keeps its LAYOUT — its indent (spaces
    or tabs, read off the first indented line), its line endings (CRLF or LF) and its
    final newline or the lack of one, so a wire followed by an unwire returns it
    byte-identical when it was written the way `JSON.stringify` writes. A file that is
    new, empty or on one line gets two spaces, LF and a final newline. Anything a parse
    cannot carry — a hand-aligned array, a blank line between sections — is still lost
    to the re-serialisation; that is the cost of editing JSON, and the backup holds it.*
23. **[M] A settings file this cannot read is a REFUSAL that changes nothing.** Bytes that
    ...
    *Amended 2026-09-23 (review n2): three unparsable shapes are NAMED rather than handed
    over in the parser's words — a byte-order mark, comments (`//`, `/* */`) and a comma
    before a closing bracket — each with what to do about it. Still refusals: this writes
...
28. **[M] The whole plan — every path that will move or go, with its size — is PRINTED
    ...
    *Amended 2026-09-23 (finding #26, the owner's 0.2.0 trial: 6.7 MB in the delete plan,
    ...
    `lstat` and `readdir` only — the park arm still never opens the store. The cause was
    the pre-flight itself: `claude mcp list` starts our MCP server as a health check, and
    that server folds the store's write-ahead log into the database on its way out, so the
    first run was sized before that happened and the second after. The number is every
    byte under the path, the log included; when the log is a megabyte or more and a tenth
    or more of the store, both plans say so in the same two lines, because it is the part
