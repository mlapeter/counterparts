...
**2026-09-04, declared before merge — `fix/revision-successor-survives` (§I8: sleep's dedup archived a revision's successor into the challenger it was minted from). Class: RED-LINE FIX (§5 G12's second class, the G10 route). NOT "identical".**

- Surface-set hash, computed with `tools/parallel/surface.ts#surfaceSetHash()` on this branch (master `5c8c056` plus this diff, nothing else): `800a9a9421cd969f`. Per #41's record the same value stood on master and in `run.json`; `run.json` is NOT re-read here — the run directory is off-limits to the session that made this change, and the preflight compares it on the next daily anyway.
- **A hash match does NOT make this class "identical", and it is recorded here so nobody reads it that way.** The hash covers FIELD NAMES on three record shapes — the per-turn surfacing decision, gate-chunk, band-transition — and this change adds no field to any of them. What it changes is *what the store still holds after a revision*: before, the successor was archived `merged` at the next dedup pass and the belief left the live set; after, the successor stays live and `sleep.merged.<id>` is not written for it. Downstream of that, recall has one more live row to consider and one fewer archived one, and the wake and slice renders show a belief where they showed nothing. No surfacing FIELD moved; the surfaced CONTENT can.
- Live-host reach: only stores where a dedup pass runs while a revision's successor and its challenger are both live — in practice any cycle after a crossing, since the pair stays live and identical until something merges it. Nothing is repaired retroactively: a successor already archived `merged` stays archived, with its prose, its id and its merge record intact. **The owner's live store has had 1 revision declaration with 0 effect as of parallel-run day 1** — zero rows with `superseded_by`, zero `revision.pressure` events, so no successor exists there and this change moves nothing on it today. It becomes reachable the first time a declaration crosses the bar.
- Measured on the demo seeder (`tools/demo/seed.ts`, 30 lived days): `merged` 3 → 1. Two of the three "duplicates" were the two revisions' successors — the belief arm and the current-state arm — and the third is a genuine duplicate that still merges.
- Consequence PROPOSED, for the owner to rule (G12 says the class is recorded, never argued after): under the red-line class the clock restarts only for the criteria whose surface set moved, and none did — so day count and ratings would CARRY, on the ground that the live store holds no successor for this to change. A reviewer comparing stores across the fix should expect one extra live schema row per crossing from here on.
