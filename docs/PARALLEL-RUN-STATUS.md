...
- **Live reach — arithmetic, not a measurement.** Every `appendEvent` site passes a `day` taken from `livedDay()` at write time; `advanceClock` refuses to move backwards; `tools/migrate` writes no events. The live store has lived at most the calendar days since 2026-08-25 (eleven), and the cutoff is `livedDay − 90`, so **the first pass on the live store should delete 0 rows**, whatever v1's `activeDay` set the clock to, and keep deleting 0 until the store has lived 90 days past its first v2 event. The table's actual contents are UNVERIFIED — no agent opened the store — and `counterparts verify --dir ~/.counterparts/store` prints the count the next pass would take; **read it before merging**. A non-zero `past the window` there refutes this paragraph, not the other way round.
- **Consequence claimed: the phase clock restarts with the day's single restart.** No separate `restart.ts` command is owed by this PR; the session that made the change may not run it.
