...
**2026-09-04, declared before merge — `fix/revision-successor-survives` (§I8: sleep's dedup archived a revision's successor into the challenger it was minted from). Class: BEHAVIOUR-CHANGING, NOT identical.**

- Surface-set hash, computed with `tools/parallel/surface.ts#surfaceSetHash()`: master `5c8c056` → `800a9a9421cd969f`; branch `fix/revision-successor-survives` → `800a9a9421cd969f`; `run.json` (read-only) records `800a9a9421cd969f`. **A hash match does NOT make this class IDENTICAL, and it is recorded here so nobody reads it that way.** The hash covers FIELD NAMES on three record shapes — the per-turn surfacing decision, gate-chunk, band-transition — and this change adds no field to any of them. What it changes is *what the store still holds after a revision*: before, the successor was archived `merged` at the next dedup pass and the belief left the live set; after, the successor stays live and `sleep.merged.<id>` is not written for it. Downstream of that, recall has one more live row to consider and one fewer archived one, and the wake/slice renders show a belief where they showed nothing. No surfacing FIELD moved; the surfaced CONTENT can.
...
- Consequence claimed: day count and ratings CARRY (no surfacing field moved, and the live store has no successor to change), but this is a BEHAVIOUR declaration, not an identity one: a reviewer comparing stores across the fix should expect one extra live schema row per crossing from here on.
