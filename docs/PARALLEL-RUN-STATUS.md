...
- **Live reach, and why "no live row is affected" is NOT claimable.** Any cycle in which an element and an ordinary memory carry the same body. Migration makes that likely rather than exotic: `tools/migrate/apply.ts#writeElement` minted every migrated element at the IMPORT day while migrated memories kept their v1 `birthDay`, so on every such pair the memory is strictly older and the element loses without even needing the `mem_` < `sch_` tie-break. The day-1 record says nothing either way about schema/memory collisions, and this session cannot open that store, so the honest class is the third one.
