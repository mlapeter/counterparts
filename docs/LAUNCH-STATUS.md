...
| G49 | **taxscrub capture.** The owner asked whether `~/random/taxscrub` (a local SSN/EIN scrubber for tax documents, meant never to pass real data through an LLM) should be captured at all. Checked 2026-09-14: its four crashed spans were swept at this session's first Stop (15 memories, all about the tool's design); the only identifier shapes in the whole session are the tool's fixtures (`123-45-6789` family), one of which sits in `spans/df02a1de17c7/assistant.jsonl` inside a sentence saying they are fixtures; none in prose or sqlite. Recommendation: the observer pattern the bookkeeping dirs use (project `.claude/settings.json` env → an observer config + `COUNTERPARTS_OBSERVER=1`), written only on the owner's word. **Done 2026-09-14 on the owner's word:** the tool now lives at `~/taxscrub` (a public-release repo; `~/random/taxscrub` is gone), so `~/taxscrub/.claude/settings.json` + `counterparts-observer.json` were written identical to the bookkeeping pattern and `.claude/` added to its `.gitignore` (uncommitted there — the owner's repo). The stale `~/random/taxscrub` scope entry in `spans/scopes.json` is harmless. | done |

### Closed the same afternoon, on the owner's permission
...
| G58 | **`recall`'s model-facing `claim` is now false twice, and the fixes differ.** `tools.ts` says "It writes nothing and trains nothing … nothing you look at here gets stronger for having been looked at". After #112 it writes host bookkeeping (safe correction: "writes host bookkeeping and no memory"); since #99 looking DOES strengthen at the boundary, and saying so hands the model a lever on its own reinforcement signal. The `claim` / `mechanizedBy` pair is the repo's honesty contract. Two rulings; the write half is the easy yes. Also from the review: the handle key is an unsalted sha256 prefix — a membership oracle for guessed titles in a directory documented as carrying no content; a per-store salt is one line. **RULED 2026-09-16** — "Exposure never strengthens. Retrieval always does." Both halves done on #112: the `claim` now says a search strengthens nothing and an EXPANSION is credited at the boundary once per lived day, with `mechanizedBy` pointing at `recall/index.ts#build` for the search half and `expansions.ts` → `hooks.ts#creditAtBoundary` → `recall/index.ts#resolveUse` for the credit half; and the key is salted per store (`sessions/expansions.salt`, minted `wx`, fails closed). The old absolute survives in `core/recall/CONTRACT.md` — §3 Keeps (the deliberate-recall bullet, "**Ranking is not recording**: it trains nothing and deposits nothing") and §5 Contract guarantee 8 — and is the one place left to correct; core was out of #112's scope. | ruled; done on #112 |
